...
class ReporteProductoModelo extends FPDF {
    ...
    public function infoReporte() {
        ...
        $this->Cell(0, 6, 'Total de productos: ' . $this->datosReporte['total_registros'], 0, 1);
        $this->Ln(5);
    }
    
    public function tablaProductos() {
        $productos = $this->datosReporte['productos'];
        $azul_principal = array(41, 128, 185);
        ...
        $this->Cell(35, 8, 'Valor Total', 1, 1, 'C', true);
        ...
        foreach ($productos as $producto) {
            ...
            // Verificar que stock y costo sean numéricos
            $stock = is_numeric($producto['stock']) ? $producto['stock'] : 0;
            $costo = is_numeric($producto['costo']) ? $producto['costo'] : 0;
            $valorTotal = $costo * $stock;
            $valorTotalInventario += $valorTotal;
            
            $this->Cell(15, 7, $contador, 1, 0, 'C', $fill);
            $this->Cell(60, 7, $this->truncateText($producto['nombre'], 35), 1, 0, 'L', $fill);
            $this->Cell(25, 7, $stock, 1, 0, 'C', $fill);
            $this->Cell(25, 7, $producto['unidad_medida'], 1, 0, 'C', $fill);
            $this->Cell(30, 7, number_format($costo, 2, ',', '.'), 1, 0, 'R', $fill);
            $this->Cell(35, 7, number_format($valorTotal, 2, ',', '.'), 1, 1, 'R', $fill);
    ...
    public function generarPDF($datos) {
        ...
        $this->Output('I', 'Inventario_Productos_' . date('Y-m-d') . '.pdf');
        exit;
    }
    ...
    public function obtenerDatosProductos() {
        ...
        try {
            ...
            error_log("Error en ReporteProductoModelo->obtenerDatosProductos(): " . $e->getMessage());
            throw new Exception("Error al obtener los datos de productos: " . $e->getMessage());
        }
    }
}
?>